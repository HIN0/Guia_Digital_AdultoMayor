...
class ProgresoLeccion(Base):
    ...
    leccion_id = Column(Integer, ForeignKey("leccion.id"), nullable=False)
...
class IntentoQuiz(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    usuario_id = Column(Integer, ForeignKey("usuario.id"), nullable=False)
    quiz_id = Column(Integer, ForeignKey("quiz_final.id"), nullable=False)
...
class InsigniaObtenida(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    usuario_id = Column(Integer, ForeignKey("usuario.id"), nullable=False)
    insignia_id = Column(Integer, ForeignKey("insignia.id"), nullable=False)
